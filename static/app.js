...
function setPeriod(period) {
  ...
  document
    .querySelectorAll(".time-period button")
...
function setProvider(provider, model = "") {
  ...
  currentModel = model;
  document
    .querySelectorAll(".model-select button")
    .forEach((btn) => btn.classList.remove("active"));

  if (provider === "anthropic") {
    document.getElementById("btn-anthropic").classList.add("active");
  } else if (provider === "openai" && model === "o3-mini") {
    document.getElementById("btn-o3").classList.add("active");
  } else {
    document.getElementById("btn-openai").classList.add("active");
  }
}
...
async function getRecommendation(ticker) {
  ...
  container.innerHTML =
    '<div class="loading"><div class="spinner"></div>Getting recommendation...</div>';

  let url = `/api/recommendation/${ticker}?provider=${currentProvider}`;
  if (currentModel) {
